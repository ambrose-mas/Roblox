...
weightToggle.Text = "Weight: OFF [B]"
...
noclipToggle.Text = "Noclip: OFF [M]"
...
-- Biến lưu trạng thái nhấn phím
local isShiftHeld = false
local keyStates = {
	V = {pressedAt = 0, isHolding = false},
	B = {pressedAt = 0, isHolding = false},
	N = {pressedAt = 0, isHolding = false}
}
-- Tốc độ tăng giảm bằng phím (đơn vị/giây)
local ADJUST_RATE = 500 
...
local function applyWeight()
	...
	if not weightForce or weightForce.Parent ~= root then
		if weightForce then
			pcall(function() weightForce:Destroy() end)
		end
...
-- ================= HÀM CẬP NHẬT GIAO DIỆN KÉO THẢ & PHÍM TẮT ================= --

local function updateSpeedUI(val)
	speedValue = math.clamp(val, 0, 1000)
	local p = speedValue / 1000
	slider.Position = UDim2.new(p, -8, 0.5, -8)
	speedText.Text = "Speed: " .. math.floor(speedValue)
end

local function updateWeightUI(val)
	weightValue = math.clamp(val, 0, 1000)
	local p = weightValue / 1000
	weightSlider.Position = UDim2.new(p, -8, 0.5, -8)
	weightText.Text = "Weight: " .. math.floor(weightValue)
end

local function updateFlyUI(val)
	flySpeed = math.clamp(val, 0, 1000)
	local p = flySpeed / 1000
	flySlider.Position = UDim2.new(p, -8, 0.5, -8)
	flyText.Text = "Fly Speed: " .. math.floor(flySpeed)
end
...
-- Tách logic bật tắt thành function để gọi từ phím tắt
local function doToggleSpeed()
	enabled = not enabled
	toggleBtn.Text = enabled and "Speed: ON [V]" or "Speed: OFF [V]"
	toggleColor(toggleBtn, enabled)
end

local function doToggleWeight()
	weightEnabled = not weightEnabled
	weightToggle.Text = weightEnabled and "Weight: ON [B]" or "Weight: OFF [B]"
	toggleColor(weightToggle, weightEnabled)
end

local function doToggleFly()
	flyEnabled = not flyEnabled
	flyToggle.Text = flyEnabled and "Fly: ON [N]" or "Fly: OFF [N]"
...
local function doToggleNoclip()
	noclipEnabled = not noclipEnabled
	noclipToggle.Text = noclipEnabled and "Noclip: ON [M]" or "Noclip: OFF [M]"
...
toggleBtn.MouseButton1Click:Connect(doToggleSpeed)
weightToggle.MouseButton1Click:Connect(doToggleWeight)
flyToggle.MouseButton1Click:Connect(doToggleFly)
noclipToggle.MouseButton1Click:Connect(doToggleNoclip)
...
loopConnection = RunService.RenderStepped:Connect(function(dt)
	...
	-- Xử lý giữ phím tăng giảm thông số UI
	local rate = ADJUST_RATE * dt
	if isShiftHeld then rate = -rate end

	if keyStates.V.isHolding and (tick() - keyStates.V.pressedAt > 0.2) then
		updateSpeedUI(speedValue + rate)
	end
	if keyStates.B.isHolding and (tick() - keyStates.B.pressedAt > 0.2) then
		updateWeightUI(weightValue + rate)
	end
	if keyStates.N.isHolding and (tick() - keyStates.N.pressedAt > 0.2) then
		updateFlyUI(flySpeed + rate)
	end
end)
...
local inputBeganConn = UIS.InputBegan:Connect(function(input, gameProcessed)
	...
	-- Bật/Tắt Menu bằng F5 hoặc Pause
	...
	if input.KeyCode == Enum.KeyCode.V then
		keyStates.V.pressedAt = tick()
		keyStates.V.isHolding = true
	elseif input.KeyCode == Enum.KeyCode.B then
		keyStates.B.pressedAt = tick()
		keyStates.B.isHolding = true
	elseif input.KeyCode == Enum.KeyCode.N then
		keyStates.N.pressedAt = tick()
		keyStates.N.isHolding = true
	elseif input.KeyCode == Enum.KeyCode.M then
		-- Noclip không có range, nên bấm là kích hoạt luôn
		doToggleNoclip()
	end
end)
...
local inputEndedConn = UIS.InputEnded:Connect(function(input)
	...
	-- Xác định xem đây là nhấn ngắn (Toggle) hay giữ lâu (Tăng/Giảm range)
	if input.KeyCode == Enum.KeyCode.V then
		keyStates.V.isHolding = false
		if tick() - keyStates.V.pressedAt < 0.2 then
			doToggleSpeed()
		end
	elseif input.KeyCode == Enum.KeyCode.B then
		keyStates.B.isHolding = false
		if tick() - keyStates.B.pressedAt < 0.2 then
			doToggleWeight()
		end
	elseif input.KeyCode == Enum.KeyCode.N then
		keyStates.N.isHolding = false
		if tick() - keyStates.N.pressedAt < 0.2 then
			doToggleFly()
		end
	end
end)
...
-- XỬ LÝ SLIDERS (Kéo thả bằng chuột/cảm ứng)
